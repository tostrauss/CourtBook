...
import { Link } from 'react-router-dom'; // If linking to individual announcement pages later
import { Bell, Info, AlertTriangle as AlertTriangleIcon, Calendar as CalendarIcon, ChevronDown, ChevronUp, Pin, Filter, Tag } from 'lucide-react'; // Renamed AlertCircle to AlertTriangleIcon
...
import Modal from '../components/common/Modal'; // For viewing full announcement

const Announcements = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [typeFilter, setTypeFilter] = useState('');
  const [expandedAnnouncementId, setExpandedAnnouncementId] = useState(null);
  const [viewingAnnouncement, setViewingAnnouncement] = useState(null); // For modal
  
  const limit = 8; // Announcements per page
  ...
  const { data: announcementsData, isLoading, error } = useQuery(
    ...
    () => announcementService.getAnnouncements({ 
      ...
      page: currentPage, 
      limit,
      isActive: 'true', // Only show active announcements on the public page
      sort: '-isPinned,-priority,-publishDate' // Pinned first, then by priority, then by date
    }),
    { 
  ...
  const totalAnnouncements = announcementsData?.count || 0;

  const getTypeIconAndColor = (type) => {
    switch (type) {
      case 'urgent':
        return { Icon: Bell, color: 'text-error-500', bgColor: 'bg-error-50', badgeColor: 'badge-error' };
      case 'warning':
      case 'maintenance':
        return { Icon: AlertTriangleIcon, color: 'text-warning-500', bgColor: 'bg-warning-50', badgeColor: 'badge-warning' };
      case 'event':
        return { Icon: CalendarIcon, color: 'text-sky-500', bgColor: 'bg-sky-50', badgeColor: 'badge-info' }; // Using info for event
      case 'info':
      default:
        return { Icon: Info, color: 'text-primary-500', bgColor: 'bg-primary-50', badgeColor: 'badge-primary' };
    }
  };
  
  const toggleExpandAnnouncement = (announcementId) => {
    setExpandedAnnouncementId(expandedAnnouncementId === announcementId ? null : announcementId);
  };

  const handleViewAnnouncement = (announcement) => {
    setViewingAnnouncement(announcement);
    // Potentially mark as read or increment view count if backend supports it
      ...
      {!isLoading && !error && announcements.length > 0 && (
        <div className="space-y-5">
          {announcements.map((announcement) => {
            const { Icon, color, bgColor, badgeColor } = getTypeIconAndColor(announcement.type);
            const isExpanded = expandedAnnouncementId === announcement._id;
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mb-2">
                        Posted on {format(publishedDate, 'MMMM d, yyyy HH:mm')}
                        {announcement.author && ` by ${announcement.author.firstName || announcement.author.username}`}
                      </p>
                      <div 
                        className={`text-sm text-gray-600 leading-relaxed prose prose-sm max-w-none ${!isExpanded ? 'line-clamp-3' : ''}`} 
        ...
        {viewingAnnouncement && (
            <div className="space-y-3">
                <div className="flex items-center space-x-3 mb-3">
                    <div className={`flex-shrink-0 p-2 rounded-full ${getTypeIconAndColor(viewingAnnouncement.type).bgColor}`}>
                        <getTypeIconAndColor(viewingAnnouncement.type).Icon className={`h-5 w-5 ${getTypeIconAndColor(viewingAnnouncement.type).color}`} />
                    </div>
                    <span className={`badge ${getTypeIconAndColor(viewingAnnouncement.type).badgeColor} text-xs capitalize`}>{viewingAnnouncement.type}</span>
                ...
                <p className="text-xs text-gray-500">
                    Posted on {format(parseISO(viewingAnnouncement.publishDate), 'MMMM d, yyyy HH:mm')}
